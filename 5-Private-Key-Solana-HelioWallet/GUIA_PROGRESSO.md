...
```bash
uv run automacao_solana_saldo.py frases.txt 2>&1 | head -20
```
...
1. Execute uma vez: `uv run automacao_solana_saldo.py frases.txt`
...
uv run automacao_solana_saldo.py frases_retry.txt --reset
```
...
```bash
uv run automacao_solana_saldo.py novo_arquivo.txt --reset
```
