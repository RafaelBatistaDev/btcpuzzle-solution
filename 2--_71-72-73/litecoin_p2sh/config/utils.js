/**
 * Utilitários Criptográficos Litecoin P2SH-P2WPKH (BIP49)
 */
...
export class CryptoEngine {
  /**
   * Converte privkey para endereço Litecoin P2SH-P2WPKH (M...) — prefixo 0x32
   */
  static privkeyToP2SH(privkeyInt) {
    ...
    const payload  = Buffer.concat([Buffer.from([0x32]), scriptHash160]);
  ...
  /**
   * Valida endereço Litecoin P2SH (M...)
   */
  static isValidP2SHAddress(addr) {
    if (!addr || typeof addr !== 'string') return false;
    return /^M[a-km-zA-HJ-NP-Z1-9]{25,34}$/.test(addr);
  }
