/**
 * Litecoin Puzzle Solver (P2SH-P2WPKH) - Sequencial com checkpoint
 */
import fs   from 'fs';
...
// Rate limiter compartilhado entre P2PKH e P2SH (via litecoin_rate_limiter.js)
import { globalLimiter } from '../../litecoin_rate_limiter.js';
export { globalLimiter };

// ─── LitecoinSolver ───────────────────────────────────────────────────────────
export class LitecoinSolver {
  ...
  async queryRPC(addresses) {
    ...
    this.state.dailyRequests.count += addresses.length;
    this._saveState();

    try {
      return await queryLitecoinBalances({
        baseUrl,
        addresses,
  ...
  async processBatch() {
    ...
    for (const item of this.batch) {
      ...
      const record = {
        ...
        finalBalanceLtc: (balanceNum / 1e8).toFixed(8),
        totalReceived:   info.totalReceived || 0,
        totalReceivedLtc:((info.totalReceived || 0) / 1e8).toFixed(8),
        totalSent:       info.totalSent     || 0,
        totalSentLtc:    ((info.totalSent   || 0) / 1e8).toFixed(8),
        nTx:             info.nTx           || 0,
        formats: {
          BIP49L: {
            address: item.addr,
            balance: balanceNum,
            ltc:     (balanceNum / 1e8).toFixed(8),
            txCount: info.nTx || 0,
          },
        },
      };
      ...
      if (info.balance > 0n) {
        const balanceLtc = (balanceNum / 1e8).toFixed(8);
        this.log(`💰 SALDO ENCONTRADO: ${item.addr} (${balanceLtc} LTC)`);
  ...
  _saveFound(item, balance, nTx = 0) {
    const balanceLtc    = (Number(balance) / 1e8).toFixed(8);
    ...
    const content =
      `LITECOIN PUZZLE #${this.puzzleId} — ${status}\n` +
      `Endereço: ${item.addr}\n` +
      `Saldo: ${balanceLtc} LTC (${balance.toString()} litoshi)\n` +
      `Transações: ${nTx}\n` +
    ...
    const record = {
      ...
      formatted_balance:  `${balanceLtc} LTC (${balance.toString()} litoshi)`,
      nTx,
  ...
  fillBatch() {
    while (this.batch.length < RUNTIME_CONFIG.BATCH_SIZE) {
      ...
      const addr = CryptoEngine.privkeyToP2SH(privkey);
      const wif  = CryptoEngine.privkeyToWif(privkey);

      if (!CryptoEngine.isValidP2SHAddress(addr)) continue;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const solvers = ACTIVE_PUZZLES.map(id => new LitecoinSolver(id));
