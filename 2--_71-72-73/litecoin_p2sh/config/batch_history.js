...
/**
 * Litecoin P2SH Batch History Checker
...
function checkLitecoinBatchHistory() {
  ...
  const reportFile  = path.join(reportsDir, `litecoin_p2sh_report_${timestamp}.txt`);

  const allResults  = {};
  const reportLines = [];

  reportLines.push('='.repeat(80));
  reportLines.push('LITECOIN P2SH BATCH HISTORY REPORT');
        ...
        try {
          ...
          if (balance !== 0 || (data.nTx || 0) > 0) {
            foundRecords.push({
              ...
              addr:    data.address || data.formats?.BIP49L?.address || data.formats?.BIP49?.address || '',
  ...
  console.log(`\n📄 Relatório salvo: reports/litecoin_p2sh_report_${timestamp}.txt`);
