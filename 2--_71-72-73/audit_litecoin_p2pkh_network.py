#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Auditoria Litecoin P2PKH — espelha litecoin_p2pkh/config/solver.js"""

import sys
import time

from audit_common import (
    AuditResult,
    audit_cache_files,
    audit_env_keys,
    audit_puzzle_targets,
    banner,
    detect_litecoin_provider,
    env_int,
    err,
    info,
    load_env,
    ok,
    print_litecoin_result,
    print_summary,
    project_root,
    query_litecoin_balance,
    run_rate_limit_probe,
    section,
    warn,
)

LTC_P2PKH_DEFAULTS = {
    71: "LR3gVmNE5FSdxVr9p4JJXv9wxxPKzNGfez",
    72: "LSAJD1eEJVnxBnP69ZA92A5bwYwrK38xcw",
    73: "LXRn1SZgfX7nCetcEKq9euWMaPQLetNAVi",
}


def main() -> int:
    banner("AUDITORIA LITECOIN P2PKH — STACK REAL DO PROJETO")
    result = AuditResult(network="litecoin_p2pkh")
    env = load_env()
    root = project_root()

    base_url = (
        env.get("LTC_BLOCKCHAIN_INFO_BASE_URL")
        or "https://litecoinspace.org"
    ).rstrip("/")
    timeout = env_int(env, "LTC_P2PKH_TIMEOUT_MS", "LTC_TIMEOUT_MS", "TIMEOUT_MS", default=3000) / 1000.0
    delay_ms = env_int(
        env, "LTC_P2PKH_DELAY_MS", "LTC_DELAY_MS", "DELAY_MS", default=1200
    )
    batch_size = env_int(env, "LTC_P2PKH_BATCH_SIZE", "BATCH_SIZE", default=20)
    provider = detect_litecoin_provider(base_url)

    info(f"Provedor detectado: {provider}")
    info(f"Endpoint: {base_url}")
    info(f"Timeout:  {timeout * 1000:.0f}ms  |  Delay: {delay_ms}ms  |  Batch: {batch_size}")

    audit_env_keys(
        result,
        env,
        required=[
            "LTC_BLOCKCHAIN_INFO_BASE_URL",
            "LTC_P2PKH_TARGET_71",
            "LTC_P2PKH_TARGET_72",
            "LTC_P2PKH_TARGET_73",
        ],
        optional=[
            "LTC_P2PKH_DELAY_MS", "LTC_P2PKH_BATCH_SIZE", "LTC_P2PKH_TIMEOUT_MS",
            "LTC_P2PKH_MAX_REQ_24H", "LTC_DELAY_MS",
        ],
    )

    audit_cache_files(result, root / "litecoin_p2pkh")
    targets = audit_puzzle_targets(result, env, "LTC_P2PKH", LTC_P2PKH_DEFAULTS)

    section("CONECTIVIDADE COM A API")
    probe_addr = targets[71]
    r = query_litecoin_balance(base_url, probe_addr, timeout, env)
    if r["ok"]:
        result.ok(f"conectividade via {provider}")
        ok(f"API respondeu em {r['ms']:.0f}ms  (puzzle #71)")
    else:
        result.fail(f"conectividade falhou: HTTP {r['status']}")
        err(f"Falha ao consultar puzzle #71: HTTP {r['status']}")
        return print_summary(result)

    section("VALIDAÇÃO DO FORMATO DE RESPOSTA (solver.js)")
    if provider == "litecoinspace":
        ok("Campos esperados: chain_stats.*, mempool_stats.*")
    elif provider == "blockcypher":
        ok("Campos esperados: balance, final_balance, n_tx")

    section("SALDOS DOS PUZZLES 71, 72, 73 (API REAL)")
    latencies: list[float] = []
    for i, (pid, addr) in enumerate(targets.items()):
        if i > 0 and delay_ms > 0:
            time.sleep(delay_ms / 1000.0)
        r = query_litecoin_balance(base_url, addr, timeout, env)
        print_litecoin_result(f"Puzzle #{pid}", r)
        if r["ok"]:
            result.ok(f"puzzle #{pid}")
            latencies.append(r["ms"])
        else:
            result.fail(f"puzzle #{pid}: HTTP {r['status']}")

    if latencies:
        info(f"Latência média: {sum(latencies) / len(latencies):.0f}ms")

    section(f"SIMULAÇÃO DE BATCH ({batch_size} endereços)")
    genesis = "LR3gVmNE5FSdxVr9p4JJXv9wxxPKzNGfez"
    batch_addrs = [targets[p] for p in (71, 72, 73)]
    while len(batch_addrs) < batch_size:
        batch_addrs.append(genesis)

    t0 = time.time()
    batch_ok = 0
    for i, addr in enumerate(batch_addrs[:batch_size]):
        if i > 0 and delay_ms > 0:
            time.sleep(delay_ms / 1000.0)
        r = query_litecoin_balance(base_url, addr, timeout, env)
        if r["ok"]:
            batch_ok += 1
        if r["status"] == 429:
            warn(f"Rate limit no endereço {i + 1} — interrompendo batch")
            result.warn("rate limit no batch simulado")
            break
    elapsed = (time.time() - t0) * 1000
    ok(f"Processados: {batch_ok}/{batch_size}  em {elapsed:.0f}ms")
    if batch_ok == batch_size:
        result.ok("batch simulado")

    def _probe() -> tuple[int, object]:
        r = query_litecoin_balance(base_url, probe_addr, timeout, env)
        status = 200 if r["ok"] else (r["status"] if isinstance(r["status"], int) else 0)
        return (status, r)

    run_rate_limit_probe(_probe, bursts=5, result=result)

    return print_summary(result)


if __name__ == "__main__":
    sys.exit(main())
