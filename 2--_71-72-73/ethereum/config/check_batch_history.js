...
/**
 * Ethereum Batch History Checker
...
function checkEthereumBatchHistory() {
  ...
  const reportFile = path.join(reportsDir, `ethereum_report_${timestamp}.txt`);
  
  const allResults = {};
  const reportLines = [];
  
  reportLines.push('='.repeat(80));
  reportLines.push('ETHEREUM BATCH HISTORY REPORT');
  ...
  for (const puzzle of puzzles) {
    ...
    try {
      ...
      if (foundRecords.length > 0) {
        ...
        reportLines.push(`${'Linha'.padEnd(8)} ${'Endereço'.padEnd(45)} ${'Saldo (ETH)'.padEnd(15)}`);
        reportLines.push('-'.repeat(80));
  ...
  console.log(`\n📄 Relatório salvo: reports/ethereum_report_${timestamp}.txt`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  checkEthereumBatchHistory();
}

export { checkEthereumBatchHistory };
