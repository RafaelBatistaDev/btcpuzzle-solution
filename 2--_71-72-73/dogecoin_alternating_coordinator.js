#!/usr/bin/env node
/**
 * Coordenador Dogecoin — alterna lotes P2PKH ↔ P2SH-P2WPKH por puzzle.
 */

import config from './config.js';
import { DogecoinSolver as P2PKHSolver } from './dogecoin_p2pkh/config/solver.js';
import { DogecoinSolver as P2SHSolver } from './dogecoin_p2sh/config/solver.js';
import { ACTIVE_PUZZLES, RUNTIME_CONFIG as P2PKH_RUNTIME } from './dogecoin_p2pkh/config/config.js';
import { RUNTIME_CONFIG as P2SH_RUNTIME } from './dogecoin_p2sh/config/config.js';
import { initLimiterDelay } from './dogecoin_rate_limiter.js';

const sharedDelay = Number(
  process.env.DOGE_DELAY_MS ||
...
async function runAlternatingForPuzzle(puzzleId) {
  ...
  while (!p2pkhDone || !p2shDone) {
    ...
    if (cycle % 10 === 0) {
      ...
      console.log(
        `[DOGE P${puzzleId}] ciclo ${cycle} | P2PKH ${p2pkhPct}% | P2SH ${p2shPct}% | ` +
...
process.on('SIGINT', () => {
  saveAll();
  console.log('\n✅ Estado Dogecoin salvo. Encerrando.');
  process.exit(0);
});
...
console.log('\n╔════════════════════════════════════════════════════════════╗');
console.log('║  🚀 DOGECOIN ALTERNADO — P2PKH ↔ P2SH por lote             ║');
console.log(
...
Promise.all(
  ...
  .then(() => {
    console.log('🏁 Todos os puzzles Dogecoin (P2PKH + P2SH-P2WPKH) processados.');
    process.exit(0);
  })
  .catch((err) => {
    console.error('❌ Erro no coordenador Dogecoin:', err);
    saveAll();
    process.exit(1);
  });
