...
# ============ BITCOIN ============
if [ $BTC_OK -eq 1 ]; then
  echo "▶️  [1/5] Bitcoin P73 (Alternado: P2PKH ↔ P2WPKH ↔ P2SH-P2WPKH)..."
  (run_puzzle_safe "BITCOIN" 73 "bitcoin_alternating_coordinator.js") &
  BTC_P73=$!
  pids+=($BTC_P73)
  record_pid "$BTC_P73"
else
...
if [ $ETH_OK -eq 1 ]; then
  ...
  sleep 3
  (run_puzzle_safe "ETHEREUM" 73 "ethereum/puzzle_solver_ethereum.js") &
  ETH_P73=$!
  pids+=($ETH_P73)
  record_pid "$ETH_P73"
else
...
if [ $SOL_OK -eq 1 ]; then
  ...
  sleep 3
  (run_puzzle_safe "SOLANA" 73 "solana/puzzle_solver_solana.js") &
  SOL_P73=$!
  pids+=($SOL_P73)
  record_pid "$SOL_P73"
else
...
if [ $POLY_OK -eq 1 ]; then
  ...
  sleep 3
  (run_puzzle_safe "POLYGON" 73 "polygon/puzzle_solver_polygon.js") &
  POLY_P73=$!
  pids+=($POLY_P73)
  record_pid "$POLY_P73"
else
...
if [ $BNB_OK -eq 1 ]; then
  echo "▶️  [5/5] BNB P73..."
  sleep 3
  (run_puzzle_safe "BNB" 73 "bnb/puzzle_solver_bnb.js") &
  BNB_P73=$!
  pids+=($BNB_P73)
  record_pid "$BNB_P73"
else
