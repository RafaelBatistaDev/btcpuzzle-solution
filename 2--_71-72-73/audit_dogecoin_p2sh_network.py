#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Auditoria Dogecoin P2SH-P2WPKH — espelha dogecoin_p2sh/config/solver.js"""
...
DOGE_P2SH_DEFAULTS = {
    71: "9u84eha9ip1vLLCbCLm1PYmtX7wHBgwqYH",
    72: "AEUpk9N3rTeUBeE24QtjsFoU1ohLV9eU4e",
    73: "9x6dEGCcGv4TtzppPwi128DdYFHHSB5Ayx",
}


def main() -> int:
    banner("AUDITORIA DOGECOIN P2SH-P2WPKH — STACK REAL DO PROJETO")
    result = AuditResult(network="dogecoin_p2sh")
    env = load_env()
    root = project_root()
    ...
    timeout = env_int(env, "DOGE_P2SH_TIMEOUT_MS", "DOGE_TIMEOUT_MS", "TIMEOUT_MS", default=3000) / 1000.0
    delay_ms = env_int(
        env, "DOGE_P2SH_DELAY_MS", "DOGE_DELAY_MS", "DELAY_MS", default=1200
    )
    batch_size = env_int(env, "DOGE_P2SH_BATCH_SIZE", "BATCH_SIZE", default=20)
    ...
    audit_env_keys(
        ...
        optional=[
            "DOGE_P2SH_DELAY_MS", "DOGE_P2SH_BATCH_SIZE", "DOGE_P2SH_TIMEOUT_MS",
            "DOGE_P2SH_MAX_REQ_24H", "DOGE_DELAY_MS",
        ],
    )

    audit_cache_files(result, root / "dogecoin_p2sh")
    targets = audit_puzzle_targets(result, env, "DOGE_P2SH", DOGE_P2SH_DEFAULTS)

    section("CONECTIVIDADE COM A API")
    probe_addr = targets[71] or DOGE_P2SH_DEFAULTS[71]
    r = query_dogecoin_balance(base_url, probe_addr, timeout, env)
    if r["ok"]:
        result.ok(f"conectividade via {provider}")
        ok(f"API respondeu em {r['ms']:.0f}ms  (probe P2SH)")
    else:
    ...
    probe = DOGE_P2SH_DEFAULTS[71]
    batch_addrs = [targets.get(p) or DOGE_P2SH_DEFAULTS[p] for p in (71, 72, 73)]
