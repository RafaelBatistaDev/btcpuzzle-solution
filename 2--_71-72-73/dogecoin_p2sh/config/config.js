/**
 * Configuração Dogecoin Puzzles 71, 72, 73 — P2SH-P2WPKH (BIP49)
 */
import fs from 'fs';
...
function resolveTarget(key) {
  ...
  if (addr && !DOGE_P2SH_REGEX.test(addr)) {
    throw new Error(`❌ ${key} deve ser endereço Dogecoin P2SH (começa com 9 ou A): "${addr}"`);
  }
  return addr;
}
...
export const PUZZLE_CONFIG = {
  71: {
    name: 'DOGE_PUZZLE_71',
    target: resolveTarget('DOGE_P2SH_TARGET_71'),
  ...
  72: {
    name: 'DOGE_PUZZLE_72',
    target: resolveTarget('DOGE_P2SH_TARGET_72'),
  ...
  73: {
    name: 'DOGE_PUZZLE_73',
    target: resolveTarget('DOGE_P2SH_TARGET_73'),
...
export const RUNTIME_CONFIG = {
  ...
  BATCH_SIZE: Number(
    process.env.DOGE_P2SH_BATCH_SIZE || envConfig.DOGE_P2SH_BATCH_SIZE ||
    process.env.BATCH_SIZE           || envConfig.BATCH_SIZE           ||
    20
  ),
  DELAY_MS: Number(
    process.env.DOGE_P2SH_DELAY_MS || envConfig.DOGE_P2SH_DELAY_MS ||
    process.env.DOGE_DELAY_MS      || envConfig.DOGE_DELAY_MS      ||
    process.env.DELAY_MS           || envConfig.DELAY_MS           ||
    2000
  ),
  INITIAL_DELAY_MS: Number(
    process.env.DOGE_P2SH_INITIAL_DELAY_MS || envConfig.DOGE_P2SH_INITIAL_DELAY_MS ||
    process.env.DOGE_INITIAL_DELAY_MS    || envConfig.DOGE_INITIAL_DELAY_MS    ||
    0
  ),
  MAX_REQ_24H: Number(
    process.env.DOGE_P2SH_MAX_REQ_24H || envConfig.DOGE_P2SH_MAX_REQ_24H ||
    process.env.DOGE_MAX_REQ_24H    || envConfig.DOGE_MAX_REQ_24H    ||
    process.env.MAX_REQ_24H         || envConfig.MAX_REQ_24H         ||
    30000
  ),
  TIMEOUT_MS: Number(
    process.env.DOGE_P2SH_TIMEOUT_MS || envConfig.DOGE_P2SH_TIMEOUT_MS ||
