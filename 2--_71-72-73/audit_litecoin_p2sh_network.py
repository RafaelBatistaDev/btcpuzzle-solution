#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Auditoria Litecoin P2SH-P2WPKH — espelha litecoin_p2sh/config/solver.js"""
...
LTC_P2SH_DEFAULTS = {
    71: "MTUnxx2JHXqEiyzNNKyYives98aJqvmBBf",
    72: "MMfYYqDjxwjKZtPgEFn93bmLDcspjHYQve",
    73: "MMDkJh4qpqGiNyMFaiV2hiezNdc7N1Kxp2",
}


def main() -> int:
    banner("AUDITORIA LITECOIN P2SH-P2WPKH — STACK REAL DO PROJETO")
    result = AuditResult(network="litecoin_p2sh")
    env = load_env()
    root = project_root()
    ...
    timeout = env_int(env, "LTC_P2SH_TIMEOUT_MS", "LTC_TIMEOUT_MS", "TIMEOUT_MS", default=3000) / 1000.0
    delay_ms = env_int(
        env, "LTC_P2SH_DELAY_MS", "LTC_DELAY_MS", "DELAY_MS", default=1200
    )
    batch_size = env_int(env, "LTC_P2SH_BATCH_SIZE", "BATCH_SIZE", default=20)
    ...
    audit_env_keys(
        ...
        optional=[
            "LTC_P2SH_DELAY_MS", "LTC_P2SH_BATCH_SIZE", "LTC_P2SH_TIMEOUT_MS",
            "LTC_P2SH_MAX_REQ_24H", "LTC_DELAY_MS",
        ],
    )

    audit_cache_files(result, root / "litecoin_p2sh")
    targets = audit_puzzle_targets(result, env, "LTC_P2SH", LTC_P2SH_DEFAULTS)
    ...
    genesis = "MTUnxx2JHXqEiyzNNKyYives98aJqvmBBf"
