...
/**
 * Dogecoin P2PKH Batch History Checker
...
function checkDogecoinBatchHistory() {
  ...
  const reportFile  = path.join(reportsDir, `dogecoin_p2pkh_report_${timestamp}.txt`);

  const allResults  = {};
  const reportLines = [];

  reportLines.push('='.repeat(80));
  reportLines.push('DOGECOIN P2PKH BATCH HISTORY REPORT');
        ...
        try {
          ...
          if (balance !== 0 || (data.nTx || 0) > 0) {
            foundRecords.push({
              ...
              addr:    data.address || data.formats?.BIP44L?.address || data.formats?.BIP44C?.address || '',
  ...
  console.log(`\n📄 Relatório salvo: reports/dogecoin_p2pkh_report_${timestamp}.txt`);

  return allResults;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  checkDogecoinBatchHistory();
}

export { checkDogecoinBatchHistory };
