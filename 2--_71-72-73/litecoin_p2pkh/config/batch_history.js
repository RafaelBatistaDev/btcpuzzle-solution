...
/**
 * Litecoin P2PKH Batch History Checker
...
function checkLitecoinBatchHistory() {
  ...
  const reportFile  = path.join(reportsDir, `litecoin_p2pkh_report_${timestamp}.txt`);

  const allResults  = {};
  const reportLines = [];

  reportLines.push('='.repeat(80));
  reportLines.push('LITECOIN P2PKH BATCH HISTORY REPORT');
  ...
  console.log(`\n📄 Relatório salvo: reports/litecoin_p2pkh_report_${timestamp}.txt`);

  return allResults;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  checkLitecoinBatchHistory();
}

export { checkLitecoinBatchHistory };
