/**
 * Polygon Puzzle Solver - Simplificado
 */
...
export class PolygonSolver {
  ...
  /**
   * Consulta RPC Gateway - MATIC Balance
   * Usando dRPC - eth_getBalance
   * Com detecção e reação a erro 429 (Rate Limit)
   * ⏰ COM ESCALONAMENTO TEMPORAL: Sem cruzamento com outros solvers
   */
  ...
  async processBatch() {
    ...
    for (const item of this.batch) {
      ...
      const record = {
        ...
        finalBalanceMatic: (Number(info.balance) / 1e18).toFixed(18),
        nTx:             0,
      };
      fs.appendFileSync(historyFile, JSON.stringify(record) + '\n');

      if (info.balance > 0n) {
        const balanceMatic = (Number(info.balance) / 1e18).toFixed(8);
        this.log(`💰 SALDO ENCONTRADO: ${item.addr} (${balanceMatic} MATIC)`);
  ...
  _saveFound(item, balance) {
    const balanceMatic = (Number(balance) / 1e18).toFixed(8);
    const privHexPadded = item.privHex.padStart(64, '0');
    const content = `POLYGON PUZZLE #${this.puzzleId} ENCONTRADO!\n` +
      `Endereço: ${item.addr}\n` +
      `Saldo: ${balanceMatic} MATIC (${balance.toString()} Wei)\n` +
    ...
    const record = {
      ...
      formatted_balance: `${balanceMatic} MATIC (${balance.toString()} Wei)`,
      privHex: privHexPadded,
      privkey_length: 64,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const solvers = PUZZLE_IDS.map(id => new PolygonSolver(id));
