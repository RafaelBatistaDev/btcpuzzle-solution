/**
 * Polygon Balance Checker - Apenas consulta saldo
...
class PolygonBalanceChecker {
        ...
        if (resp.data.result && resp.data.result.assets) {
          ...
          if (maticBalance) {
            ...
            results.push({
              ...
              balanceWei: maticBalance.balanceRawInteger,
    ...
    if (withBalance.length > 0) {
      ...
      withBalance.forEach(r => {
        const record = {
          ...
          formatted_balance: `${r.balanceEth} MATIC (${r.balanceWei} Wei)`,
          timestamp_verified: r.timestamp
        };
        fs.appendFileSync(foundFile, JSON.stringify(record) + '\n');
        this.log(`  → ${r.address}: ${r.balanceEth} MATIC`);
      });
    } else {
   ...
   */
  async run() {
    this.log(`🔍 Polygon P${this.puzzleId} - Balance Checker`);
...
const checker = new PolygonBalanceChecker(puzzleId);
